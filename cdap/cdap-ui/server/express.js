...
function makeApp(authAddress, cdapConfig, uiSettings) {
  ...
  // middleware
  try {
    router.use(serveFavicon(faviconPath));
  } catch (e) {
  ...
  router.use(frameguard({ action: 'sameorigin' }));
  ...
  router.get('/ui-config.js', function(req, res) {
  ...
  router.get('/ui-theme.js', function(req, res) {
    res.header({
      'Content-Type': 'text/javascript',
      'Cache-Control': 'no-store, must-revalidate',
    });
   ...
   **/
  router.get('/forwardMarketToCdap', function(req, res) {
  ...
  router.get('/downloadLogs', function(req, res) {
  ...
  */
  ...
  router.post('/accessToken', authentication);
  ...
  */
  router.post('/namespaces/:namespace/:path(*)', function(req, res) {
    var headers = {};
    if (req.headers) {
      headers = req.headers;
    }
  ...
  */
   ...
   */
  router.post('/updateTheme', function(req, res) {
    let authToken = req.headers.authorization;
    if (
      !req.headers['session-token'] ||
      (req.headers['session-token'] &&
